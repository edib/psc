...
hst=$(hostname)
key="/psc/${cls}/psc/templates/pgbackrest-db-site.conf.ctpl" 
...
for hst in ${pgserver_hosts=[@]};do
	...
	ssh $hst /var/lib/psc/scripts/postgres/render_template.sh $cls pgbackrest-db-site.conf.ctpl "/etc/pgbackrest.conf"
done
